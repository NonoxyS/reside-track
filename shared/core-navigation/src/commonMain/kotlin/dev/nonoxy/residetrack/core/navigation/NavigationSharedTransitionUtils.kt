...
// Divergence from KMMTemplate: this module depends on `common-ui`, which pulls Compose Multiplatform
// animation 1.10.1 onto the iOS classpath, where `PlaceHolderSize`/`placeHolderSize` were renamed to
// `PlaceholderSize`/`placeholderSize`. KMMTemplate's narrower dependency graph stays on 1.9.1
// (pre-rename), so it still uses the old spelling. Using the old spelling here is a hard compile
// error on iOS — do not "sync" these names back to the KMMTemplate version. The explicit
// `compose.multiplatform.animation` dependency in build.gradle.kts pins the same renamed API for the
// Android target.
